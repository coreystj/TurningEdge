...
namespace TurningEdge.Networking.Pipeline
{
    class Program
    {
        ...
        private static NetworkInfo<UnityServerSession> _networker;
        ...
        private static void Bind(NetworkInfo<UnityServerSession> networker)
        {
        ...
        private static void Networker_OnMessageReceivedSuccess(Session session, byte[] bytes)
        {
            ...
            if (_networker is Client<UnityServerSession>)
                ClientSend();
            else
        ...
        {
            Console.Write("Send: ");
            ((Client<UnityServerSession>)_networker).Send(Encoding.ASCII.GetBytes(Console.ReadLine()));
